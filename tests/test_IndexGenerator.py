...
class TestIndexGenerator(unittest.TestCase):
    def setUp(self):
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(
                f"Model path '{self.model_path}' does not exist, please downloaded f{self.model_name} to run within INL network."
            )
    ...
    def test_get_post_content(self):
        post = {
            "title": "Test Title",
            "bodyText": "Test Body",
            "comments": {"edges": [{"node": {"bodyText": "Test Comment"}}]},
        }
    ...
    def test_get_comment_content(self):
        comment = {
            ...
            "replies": {"edges": [{"node": {"bodyText": "Test Reply"}}]},
        }
    ...
    def test_my_file_reader(self, mock_file):
        reader = self.index_generator.MyFileReader(
            self.index_generator.get_post_content
        )
    ...
    @patch(
        ...
        read_data='{"discussions": {"edges": [{"node": {"title": "Test Title", "url": "http://example.com", "bodyText": "Test body text"}}]}}',
    )
    def test_read_documents(self, mock_open):
        file_reader = self.index_generator.MyFileReader(
            self.index_generator.get_post_content
        )
    ...
    @patch(
        ...
        return_value=[Document(text="Test Node")],
    )
    ...
    def test_create_index(
        self,
        mock_index,
        mock_simple_docstore,
        mock_simple_vectorstore,
        mock_simple_indexstore,
        mock_add_documents,
        mock_get_nodes,
        mock_embedding,
    ):
    ...
    def test_save_index(self, mock_storage_context):
        ...
        index.storage_context.persist.assert_called_once_with(
            persist_dir=Path("testdatabase")
        )

    @patch.object(
        IndexGenerator,
        "read_documents",
        return_value=[Document(text="Test Document")],
    )
    @patch.object(IndexGenerator, "save_index")
    @patch.object(IndexGenerator, "create_index")
    def test_generate_index(
        self, mock_save_index, mock_create_index, mock_read_documents
    ):
    ...
    @patch.object(IndexGenerator, "generate_index")
