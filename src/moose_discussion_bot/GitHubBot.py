...
class GitHubBot:
    ...
    def generate_solution(
        self, title: str, top_n: int, index: SimpleVectorStore, threshold: float
    ) -> str:
        if index is None:
        ...
        result: List[str] = []
        result.append(
            f"Here are some previous posts that may relate to your question: \n\n"
        )
    ...
    def query_response(self) -> None:
        ...
        if response.status_code == 200:
            ...
            for discussion in discussions:
                ...
                concise_solution = self.generate_solution(
                    title, self.top_n, self.index, self.threshold
                )
                response_body = (
                    f"Hey, @{author},\n\n"
                    f"{concise_solution}\n\n"
                ...
                if not self.dry_run:
                    ...
                    if response.status_code == 200:
                        ...
                        comment_id = response_data["data"]["addDiscussionComment"][
                            "comment"
                        ]["id"]
                        print(
                    ...
                    print(
                        f"Dry run mode: Would have replied to discussion: '{title}' with the following body:\n{response_body}"
                    )
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GitHub Bot for replying to discussions."
    )
    ...
    parser.add_argument(
        ...
        help="Top N most similar posts to retrieve.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=default_threshold,
        help="Cutoff threshold for similarity.",
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default=default_model_name,
        help="Model name for HuggingFace embedding.",
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
        help="Run the bot in dry run mode without posting comments.",
    )
